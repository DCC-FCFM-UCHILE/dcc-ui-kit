# DCC UI Kit 2.4.0 — guía de consumo
...
```
https://cdn.dcc.uchile.cl/ui-kit/2.4.0/
...
nunca más. Así una app puede quedarse en `1.0.0` mientras otra pasa a `2.4.0`, y nadie se rompe por
...
```html
<link rel="stylesheet" href="https://cdn.dcc.uchile.cl/ui-kit/2.4.0/dcc-ui.min.css">
<script src="https://cdn.dcc.uchile.cl/ui-kit/2.4.0/dcc-ui.bundle.min.js" defer></script>
```
...
**El kit no le toca nada al resto de tu página.** Sus estilos alcanzan sólo a los elementos con
clase `dcc-` y a lo que viva dentro de ellos. Puedes cargarlo antes o después de Bootstrap: da igual.

Si además quieres la tipografía y el fondo del kit en toda la página, agrégale la clase al `<body>`:

```html
<body class="dcc-root">
```

Eso es opt-in a propósito. Sin esa clase, el kit aporta componentes y nada más.

Con integridad verificada, si quieres los hashes de `SRI.txt`:

```html
<link rel="stylesheet"
      href="https://cdn.dcc.uchile.cl/ui-kit/2.4.0/dcc-ui.min.css"
      integrity="sha384-…"
      crossorigin="anonymous">
<script src="https://cdn.dcc.uchile.cl/ui-kit/2.4.0/dcc-ui.bundle.min.js"
        integrity="sha384-…"
...
Para actualizar, cambias `2.4.0` por la versión nueva en esas dos líneas del `base.html`. Nada más.
...
```
https://cdn.dcc.uchile.cl/ui-kit/2.4.0/assets/logo-dcc.svg
https://cdn.dcc.uchile.cl/ui-kit/2.4.0/assets/logo-uchile.svg
https://cdn.dcc.uchile.cl/ui-kit/2.4.0/assets/logo-fcfm.svg
https://cdn.dcc.uchile.cl/ui-kit/2.4.0/assets/logo-cna.svg
```

```html
<img src="https://cdn.dcc.uchile.cl/ui-kit/2.4.0/assets/logo-dcc.svg"
     alt="DCC — Ciencias de la Computación" width="160" height="79">
```

Son SVG, así que escalan sin perder nitidez; dales `width` y `height` para que la
página no salte mientras cargan.

> **No están cubiertos por la licencia MIT del kit.** Son marcas institucionales
> y su uso se rige por las normas gráficas de la Universidad de Chile. El código
> del kit es MIT; estos archivos no.

Para agregar uno nuevo basta dejar el archivo en `src/assets/` del repositorio y
reconstruir: lo que esté ahí se publica tal cual, y el CI verifica que `dist/`
coincida con el fuente.
